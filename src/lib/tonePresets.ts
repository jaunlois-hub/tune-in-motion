...
export const TONE_PRESETS: TonePreset[] = [
  ...
  { name: 'Sparkling Clean', artist: 'John Mayer', genre: 'Blues/Pop', description: 'Crystal clear Strat clean with subtle compression',
    ...
    settings: p({ gain: 0.65, compressor: 0.35, eqTreble: 0.62, eqMid: 0.52, reverb: 0.25, chorus: 0.12 }) },
  { name: 'Jazz Warm', artist: 'Joe Pass', genre: 'Jazz', description: 'Warm rounded jazz tone with rolled-off treble',
    ...
    settings: p({ gain: 0.55, eqBass: 0.58, eqMid: 0.55, eqTreble: 0.3, reverb: 0.2, compressor: 0.25 }) },
  { name: 'Funk Clean', artist: 'Nile Rodgers', genre: 'Funk', description: 'Tight percussive clean with compression',
    ...
    settings: p({ gain: 0.7, compressor: 0.55, eqBass: 0.38, eqMid: 0.58, eqTreble: 0.65, noiseGate: 0.35 }) },
  { name: 'Country Twang', artist: 'Brad Paisley', genre: 'Country', description: 'Bright clean with snappy compression',
    ...
    settings: p({ gain: 0.65, compressor: 0.5, eqBass: 0.38, eqTreble: 0.68, chorus: 0.08, reverb: 0.18 }) },
  { name: 'Indie Shimmer', artist: 'The Edge', genre: 'Rock', description: 'Ambient clean with delay and modulation',
    ...
    settings: p({ gain: 0.6, delay: 0.45, delayTime: 0.38, reverb: 0.5, chorus: 0.2, eqTreble: 0.62 }) },
  { name: 'Acoustic Fingerstyle', artist: 'Tommy Emmanuel', genre: 'Acoustic', description: 'Full-bodied acoustic with natural warmth',
    ...
    settings: p({ gain: 0.6, compressor: 0.3, eqBass: 0.55, eqMid: 0.55, eqTreble: 0.58, reverb: 0.22 }) },
  { name: 'Gospel Clean', artist: 'Kerry 2 Smooth', genre: 'Gospel', description: 'Bright sparkling clean with modulation',
    ...
    settings: p({ gain: 0.65, compressor: 0.4, eqTreble: 0.65, chorus: 0.22, chorusRate: 1.2, reverb: 0.35, delay: 0.15 }) },
  ...
  { name: 'Blues Crunch', artist: 'SRV', genre: 'Blues', description: 'Texas blues overdrive with warm mids',
    ...
    settings: p({ distortion: 0.28, gain: 0.75, eqBass: 0.52, eqMid: 0.62, eqTreble: 0.52, reverb: 0.2, compressor: 0.25 }) },
  { name: 'Classic Rock', artist: 'AC/DC', genre: 'Rock', description: 'Marshall-style crunch, punchy mids',
    ...
    settings: p({ distortion: 0.42, gain: 0.8, eqBass: 0.55, eqMid: 0.65, eqTreble: 0.55, noiseGate: 0.2, compressor: 0.2 }) },
  { name: 'Punk Rock', artist: 'Green Day', genre: 'Punk', description: 'Aggressive mid-forward crunch',
    ...
    settings: p({ distortion: 0.48, gain: 0.85, eqBass: 0.48, eqMid: 0.65, eqTreble: 0.58, compressor: 0.3, noiseGate: 0.3 }) },
  { name: 'Vintage Overdrive', artist: 'Jimi Hendrix', genre: 'Rock', description: 'Warm fuzz-like overdrive with psychedelic vibe',
    ...
    settings: p({ distortion: 0.45, gain: 0.7, reverb: 0.3, phaser: 0.25, phaserRate: 0.6, eqMid: 0.58, eqBass: 0.52 }) },
  { name: 'Grunge', artist: 'Nirvana', genre: 'Grunge', description: 'Raw scooped distortion with attitude',
    ...
    settings: p({ distortion: 0.55, gain: 0.8, eqBass: 0.58, eqMid: 0.35, eqTreble: 0.52, chorus: 0.08, noiseGate: 0.25 }) },
  { name: 'Blues Rock', artist: 'Gary Moore', genre: 'Blues', description: 'Singing blues-rock lead tone',
    ...
    settings: p({ distortion: 0.35, gain: 0.75, reverb: 0.3, delay: 0.18, eqMid: 0.62, eqTreble: 0.52, compressor: 0.3 }) },
  { name: 'Southern Rock', artist: 'Lynyrd Skynyrd', genre: 'Southern Rock', description: 'Open crunchy tone with warm sustain',
    ...
    settings: p({ distortion: 0.35, gain: 0.75, eqBass: 0.55, eqMid: 0.58, eqTreble: 0.52, reverb: 0.22, compressor: 0.2 }) },
  { name: 'Pop Rock', artist: 'Foo Fighters', genre: 'Rock', description: 'Radio-friendly crunch with big power chords',
    ...
    settings: p({ distortion: 0.4, gain: 0.8, eqBass: 0.52, eqMid: 0.58, eqTreble: 0.58, reverb: 0.18, compressor: 0.25 }) },
  ...
  { name: 'Metal Rhythm', artist: 'Metallica', genre: 'Metal', description: 'Tight palm-muted metal rhythm tone',
    ...
    settings: p({ distortion: 0.68, gain: 0.82, eqBass: 0.52, eqMid: 0.4, eqTreble: 0.62, noiseGate: 0.5, compressor: 0.35 }) },
  { name: 'Djent', artist: 'Periphery', genre: 'Progressive Metal', description: 'Ultra-tight modern metal with scooped mids',
    ...
    settings: p({ distortion: 0.72, gain: 0.85, eqBass: 0.55, eqMid: 0.3, eqTreble: 0.65, noiseGate: 0.6, compressor: 0.5 }) },
  { name: 'Nu-Metal', artist: 'Wes Borland', genre: 'Nu-Metal', description: 'Heavy downtuned with industrial edge',
    ...
    settings: p({ distortion: 0.62, gain: 0.82, eqBass: 0.65, eqMid: 0.42, eqTreble: 0.52, noiseGate: 0.4, flanger: 0.12 }) },
  { name: 'Shred Lead', artist: 'Steve Vai', genre: 'Rock', description: 'Smooth singing lead tone with sustain',
    ...
    settings: p({ distortion: 0.55, gain: 0.78, reverb: 0.3, delay: 0.2, delayTime: 0.35, eqMid: 0.62, compressor: 0.4 }) },
  { name: 'Doom Sludge', artist: 'Electric Wizard', genre: 'Doom', description: 'Massive fuzzy wall of sound',
    ...
    settings: p({ distortion: 0.78, gain: 0.85, eqBass: 0.68, eqMid: 0.48, eqTreble: 0.38, reverb: 0.3 }) },
  { name: 'Deathcore', artist: 'August Burns Red', genre: 'Metal', description: 'Brutal tight gain with aggressive EQ',
    ...
    settings: p({ distortion: 0.75, gain: 0.85, eqBass: 0.55, eqMid: 0.35, eqTreble: 0.65, noiseGate: 0.6, compressor: 0.45 }) },
  { name: 'Black Metal', artist: 'Dimmu Borgir', genre: 'Black Metal', description: 'Buzzy tremolo-picked raw distortion',
    ...
    settings: p({ distortion: 0.7, gain: 0.82, eqBass: 0.42, eqMid: 0.4, eqTreble: 0.65, noiseGate: 0.5, reverb: 0.12 }) },
  { name: 'Death Metal', artist: 'Death', genre: 'Death Metal', description: 'Thick brutal gain with scooped mids',
    ...
    settings: p({ distortion: 0.75, gain: 0.85, eqBass: 0.6, eqMid: 0.3, eqTreble: 0.62, noiseGate: 0.55, compressor: 0.4 }) },
  { name: 'Power Metal', artist: 'Helloween', genre: 'Power Metal', description: 'Bright high-gain with soaring lead character',
    ...
    settings: p({ distortion: 0.58, gain: 0.82, eqBass: 0.48, eqMid: 0.55, eqTreble: 0.65, delay: 0.15, reverb: 0.2, compressor: 0.3 }) },
  ...
  { name: 'Gilmour Solo', artist: 'David Gilmour', genre: 'Progressive Rock', description: 'Singing lead with lush delay and reverb',
    ...
    settings: p({ distortion: 0.25, gain: 0.68, reverb: 0.55, delay: 0.45, delayTime: 0.45, compressor: 0.3, phaser: 0.12 }) },
  { name: 'Wah Funk', artist: 'Jimi Hendrix', genre: 'Rock', description: 'Voodoo Chile style wah rhythm',
    ...
    settings: p({ distortion: 0.35, gain: 0.72, wah: 0.65, wahFreq: 0.55, eqMid: 0.58, reverb: 0.15 }) },
  { name: 'Shoegaze', artist: 'My Bloody Valentine', genre: 'Shoegaze', description: 'Dreamy wall of modulated noise',
    ...
    settings: p({ distortion: 0.35, reverb: 0.75, chorus: 0.45, chorusRate: 0.8, tremolo: 0.18, tremoloRate: 3, delay: 0.35, delayTime: 0.4, gain: 0.6 }) },
  { name: 'Tom Morello', artist: 'Tom Morello', genre: 'Rock', description: 'RATM-style aggressive tone',
    ...
    settings: p({ distortion: 0.65, gain: 0.85, eqBass: 0.48, eqMid: 0.58, eqTreble: 0.52, noiseGate: 0.4, wah: 0.25, wahFreq: 0.65 }) },
  { name: 'Reggae Clean', artist: 'Bob Marley', genre: 'Reggae', description: 'Dark clean with tight chop',
    ...
    settings: p({ gain: 0.55, eqBass: 0.4, eqMid: 0.45, eqTreble: 0.35, compressor: 0.4, reverb: 0.15 }) },
  { name: 'Surf Rock', artist: 'Dick Dale', genre: 'Surf', description: 'Drenched spring reverb with tremolo',
    ...
    settings: p({ gain: 0.7, reverb: 0.6, tremolo: 0.45, tremoloRate: 6, eqTreble: 0.62, compressor: 0.2 }) },
  { name: '80s Ballad', artist: 'Slash', genre: 'Rock', description: 'Sweet singing lead with chorus and delay',
    ...
    settings: p({ distortion: 0.38, gain: 0.72, chorus: 0.25, reverb: 0.4, delay: 0.3, delayTime: 0.35, eqMid: 0.58, compressor: 0.3 }) },
  { name: 'Lo-Fi', artist: 'Mac DeMarco', genre: 'Indie', description: 'Wobbly chorus with warm character',
    ...
    settings: p({ gain: 0.55, chorus: 0.45, chorusRate: 2, eqBass: 0.55, eqTreble: 0.35, reverb: 0.2 }) },
  { name: 'Octave Fuzz', artist: 'Jack White', genre: 'Garage', description: 'Raw octave-up distortion',
    ...
    settings: p({ distortion: 0.6, gain: 0.8, octaver: 0.45, octaverMix: 0.4, eqMid: 0.58, noiseGate: 0.3 }) },
  { name: 'Post-Rock', artist: 'Explosions in the Sky', genre: 'Post-Rock', description: 'Atmospheric swells with massive reverb',
    ...
    settings: p({ gain: 0.55, reverb: 0.7, delay: 0.5, delayTime: 0.5, tremolo: 0.12, tremoloRate: 2, chorus: 0.18, eqTreble: 0.58 }) },
  { name: 'Tame Impala', artist: 'Kevin Parker', genre: 'Psychedelic', description: 'Dreamy phased psychedelia with fuzz',
    ...
    settings: p({ distortion: 0.4, gain: 0.68, phaser: 0.4, phaserRate: 0.5, reverb: 0.4, delay: 0.25, chorus: 0.12, eqMid: 0.55 }) },
  { name: 'Arctic Monkeys', artist: 'Alex Turner', genre: 'Indie Rock', description: 'Garage-tinged indie crunch',
    ...
    settings: p({ distortion: 0.38, gain: 0.75, eqBass: 0.52, eqMid: 0.58, eqTreble: 0.52, reverb: 0.15, compressor: 0.2 }) },
  { name: 'Van Halen', artist: 'Eddie Van Halen', genre: 'Rock', description: 'Brown sound - warm high gain with phaser',
    ...
    settings: p({ distortion: 0.58, gain: 0.8, phaser: 0.18, phaserRate: 0.4, eqMid: 0.62, eqTreble: 0.58, reverb: 0.15, flanger: 0.08 }) },
  { name: 'R&B Soul', artist: 'Isaiah Sharkey', genre: 'R&B', description: 'Smooth clean R&B with warm compression',
    ...
    settings: p({ gain: 0.6, compressor: 0.45, eqBass: 0.55, eqMid: 0.55, eqTreble: 0.48, chorus: 0.08, reverb: 0.2 }) },
  { name: 'Flamenco', artist: 'Paco de Lucía', genre: 'Flamenco', description: 'Bright percussive acoustic-style tone',
    ...
    settings: p({ gain: 0.65, eqBass: 0.4, eqMid: 0.5, eqTreble: 0.65, compressor: 0.35, reverb: 0.25 }) },
  { name: 'EDM Guitar', artist: 'Martin Garrix', genre: 'EDM', description: 'Compressed clean with heavy modulation for electronic tracks',
    ...
    settings: p({ gain: 0.7, compressor: 0.55, chorus: 0.3, chorusRate: 1.8, delay: 0.3, delayTime: 0.25, eqTreble: 0.62, noiseGate: 0.3 }) },
  { name: 'Math Rock', artist: 'CHON', genre: 'Math Rock', description: 'Snappy clean with tight articulation',
    ...
    settings: p({ gain: 0.65, compressor: 0.5, eqBass: 0.42, eqMid: 0.55, eqTreble: 0.62, delay: 0.15, delayTime: 0.2, reverb: 0.15 }) },
  { name: 'Midwest Emo', artist: 'American Football', genre: 'Emo', description: 'Twinkly clean with shimmering delay',
    ...
    settings: p({ gain: 0.6, delay: 0.38, delayTime: 0.3, reverb: 0.32, chorus: 0.12, eqTreble: 0.62, compressor: 0.25 }) },
  { name: 'Stoner Fuzz', artist: 'Kyuss', genre: 'Stoner Rock', description: 'Thick fuzzy desert tone',
    ...
    settings: p({ distortion: 0.68, gain: 0.8, eqBass: 0.62, eqMid: 0.52, eqTreble: 0.42, reverb: 0.2, phaser: 0.08 }) },
  { name: 'Synthwave Guitar', artist: 'Gunship', genre: 'Synthwave', description: 'Retro 80s lead with heavy chorus and delay',
    ...
    settings: p({ gain: 0.65, distortion: 0.2, chorus: 0.45, chorusRate: 1.5, delay: 0.4, delayTime: 0.35, reverb: 0.45, eqTreble: 0.58 }) },
];
