...
import { Power, RotateCcw, Volume2, ChevronDown, ChevronUp, Minus, Plus, Search, Music, Disc3, Youtube, Save, Star, X, Pause, Play } from 'lucide-react';
...
const QUICK_PRESETS: { name: string; emoji: string; settings: Partial<EffectSettings> }[] = [
  { name: 'Clean', emoji: '✨', settings: { distortion: 0, gain: 0.65, reverb: 0.2, delay: 0, chorus: 0.1, compressor: 0.25, eqBass: 0.5, eqMid: 0.5, eqTreble: 0.58, wah: 0, flanger: 0, phaser: 0, tremolo: 0, octaver: 0 } },
  { name: 'Crunch', emoji: '🔥', settings: { distortion: 0.35, gain: 0.78, reverb: 0.15, delay: 0, chorus: 0, compressor: 0.25, eqBass: 0.52, eqMid: 0.62, eqTreble: 0.55, wah: 0, flanger: 0, phaser: 0, tremolo: 0, octaver: 0 } },
  { name: 'Lead', emoji: '🎤', settings: { distortion: 0.48, gain: 0.75, reverb: 0.35, delay: 0.22, delayTime: 0.35, chorus: 0, compressor: 0.35, eqBass: 0.5, eqMid: 0.62, eqTreble: 0.55, wah: 0, flanger: 0, phaser: 0, tremolo: 0, octaver: 0 } },
  { name: 'Metal', emoji: '🤘', settings: { distortion: 0.7, gain: 0.85, reverb: 0.1, delay: 0, chorus: 0, compressor: 0.4, noiseGate: 0.5, eqBass: 0.55, eqMid: 0.35, eqTreble: 0.62, wah: 0, flanger: 0, phaser: 0, tremolo: 0, octaver: 0 } },
  { name: 'Ambient', emoji: '🌊', settings: { distortion: 0.08, gain: 0.55, reverb: 0.65, delay: 0.5, delayTime: 0.45, chorus: 0.22, compressor: 0.2, eqBass: 0.5, eqMid: 0.5, eqTreble: 0.58, wah: 0, flanger: 0, phaser: 0.12, tremolo: 0.1, tremoloRate: 2, octaver: 0 } },
  { name: 'Blues', emoji: '🎷', settings: { distortion: 0.25, gain: 0.72, reverb: 0.22, delay: 0.1, chorus: 0, compressor: 0.25, eqBass: 0.52, eqMid: 0.6, eqTreble: 0.48, wah: 0, flanger: 0, phaser: 0, tremolo: 0, octaver: 0 } },
];
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-2">
        <div className="col-span-2">
          ...
          <input value={name} onChange={e => setName(e.target.value)} placeholder="My Custom Tone"
        ...
        <div>
          ...
          <input value={artist} onChange={e => setArtist(e.target.value)} placeholder="e.g. Metallica"
            className="w-full px-3 py-2 rounded-lg bg-secondary/50 border border-border text-sm placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-primary" />
        </div>
        <div>
          <label className="text-[10px] text-muted-foreground mb-1 block">Song Name</label>
          <input value={song} onChange={e => setSong(e.target.value)} placeholder="e.g. Master of Puppets"
            className="w-full px-3 py-2 rounded-lg bg-secondary/50 border border-border text-sm placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-primary" />
        </div>
        <div className="col-span-2">
          <label className="text-[10px] text-muted-foreground mb-1 block">Genre</label>
          <input value={genre} onChange={e => setGenre(e.target.value)} placeholder="e.g. Metal, Blues, Rock"
            className="w-full px-3 py-2 rounded-lg bg-secondary/50 border border-border text-sm placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-primary" />
        </div>
      </div>
...
export function EffectsSection() {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Guitar Effects */}
      <div className="bg-card/50 border border-border rounded-2xl p-4 space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
                  ...
                  <div className="space-y-3 pt-2">
                    <div className="flex items-center gap-2">
                      <div className="relative flex-1">
                        ...
                        <input type="text" placeholder="Search artists, tones, songs..." value={presetSearch} onChange={(e) => setPresetSearch(e.target.value)}
                    ...
                    <div className="flex gap-1 overflow-x-auto pb-1">
                      {genres.map((g) => (
                        <button key={g} onClick={() => setPresetGenre(g)}
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 mb-3">
                          {filteredCustomPresets.map((preset) => (
                            <CustomPresetCard key={preset.id} preset={preset} onApply={() => applyCustomPreset(preset)} onDelete={() => deletePreset(preset.id)} />
                          ))}
                        </div>
                      </div>
                    )}
                    <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-wider mb-1">Built-in Presets ({filteredPresets.length})</p>
